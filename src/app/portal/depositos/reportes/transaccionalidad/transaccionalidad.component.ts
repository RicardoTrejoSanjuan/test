...
@Component({
  selector: "depositos", 
  templateUrl:
  "transaccionalidad.html",
  styleUrls: [
    "../../deposito.css",
    "../../parametrizacion-contratos-daz/parametrizacion.css"
  ]
})
export class TransaccionalidadComponent extends ClassGenerica {
...
// Fin
  ...
  public consreferenciasdaz() {
    ...
    nd = nd.reverse();
   let nd2:string = nd.join('-');


   let nd4:number[] = this.fechaFin.split('-');
   nd4 = nd4.reverse();
  let nd5:string = nd4.join('-');
    ...
    super.loading(true);
    let path = '/depositoazteca/resportesdaz/reportedeposito/';
    ...
    this.service.post(parameter2, path, 3).subscribe(
      data => {
        ...
        if (object.codE === 0) {
          ...
          this.objSortercopy = object2.jsonResultado;
...
// private dateSort(_item: any):void{
//   this.objSorter.sort(function(a: any, b: any) {
//       let arrDate1: any[] = a.fechaOpera.split('-');
//       let arrDate2: any[] = b.fechaOpera.split('-');
//       let date1: Date = new Date(arrDate1.shift(), arrDate1.shift(), arrDate1.shift());
  ...
  this.service.post(parameter, path, 3).subscribe(
    data => {
      ...
      if (object.codE === 0) {
        ...
        console.log('Consulta de Coantratos');
...
if (registros >= 50) {
  ...
  if (this.objSorter !== '' && this.objSorter !== null && this.objSorter !== undefined) {
    this.jsonToCsv.generateToExcel(this.objSorter,'ReporteTransaccionalidad');
    setInterval(()=> {
