...
@Component({
    ...
    templateUrl: 'catalogos-asesor.component.html',
    styleUrls: ['../../monitoreo.component.css']
})
export class MonitoreoCatalogosAsesor extends ClassGenerica {
    @ViewChild('tablaasesor') tablaAsesor: Ng2Tables;
    ...
    public RegistrosGral: any = [];
    public strRegistros: string = '';
    ...
    public PressSearch(_evt: any): void {
        try {
            ...
            this.tablaAsesor.SetTabla(new ConfigNgTable2(this.Registros.length, 10));
        } catch (e) { }
    }
        ...
        switch (_type.toUpperCase()) {
            case 'ALTA':
                this.formulario = new FormGroup({
                    ...
                    'telefono': new FormControl('', [Validators.minLength(7), Validators.maxLength(10), Validators.required]),
                    'correo': new FormControl('', [ValidationService.emailValidator, Validators.required]),
            ...
            case 'EDITAR':
                console.log(_data);
                this.formulario = new FormGroup({
                    'idAsesor': new FormControl(_data.idAsesor, Validators.required),
                    'numEmpleado': new FormControl(_data.numEmpleado, Validators.required),
                    ...
                    'telefono': new FormControl(_data.telefono, [Validators.required, Validators.minLength(7), Validators.maxLength(10)]),
                    'correo': new FormControl(_data.correo, [ValidationService.emailValidator, Validators.required]),
    ...
    private Alta(_data: any): void {
        ...
        super.loading(true);
        this._serv.post(jsonSend, '/api/traking/asesor/alta', 3).subscribe((data: any) => {
            super.loading(false);
            console.log(JSON.stringify(data));
            try {
                if (data.codE === 0) {
    ...
    private Actualizar(_data: any): void {
        ...
        super.loading(true);
        this._serv.post(jsonSend, '/api/traking/asesor/actualizacion', 3).subscribe((data: any) => {
            super.loading(false);
            console.log(JSON.stringify(data));
            try {
                if (data.codE === 0) {
    ...
    private Delete(_data: any): void {
        ...
        super.loading(true);
        this._serv.post(jsonSend, '/api/traking/asesor/delete', 3).subscribe((data: any) => {
            super.loading(false);
            console.log(JSON.stringify(data));
            try {
                if (data.codE === 0) {
