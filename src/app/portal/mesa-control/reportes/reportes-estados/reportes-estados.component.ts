...
@Component({
  selector: 'reportes-estados',
  templateUrl: 'reportes-estados.component.html',
  styleUrls: [
...
export class MesaControlReportesEstados extends ClassGenerica implements AfterViewInit{
  ...
  cambiarTerceraGrafica(_params){
    this.segunda=false;
    this.tercera=true;
    this.nombreAnalista=_params.name;
  ...
  inicializarCounters(obj) {
    ...
    generarData(objIn){
      ...
      if (objIn.tipo === 'ES' || objIn.tipo === 'S' || objIn.tipo === 'AS' || objIn.tipo === 'EAS' || objIn.tipo === 'AES') {
          for (var i = 0; i < obj.length; i++) {
              var porc = Number(((obj[i].total * 100) / total).toFixed(2));
              data.push({name: this.statusTexto(obj[i].estadoSolicitud), y: porc, total: obj[i].total, id: obj[i].estadoSolicitud, tipo: objIn.tipo, empresa: obj[i].idEmpresa});
          }
      }
      if (objIn.tipo === 'ESA' || objIn.tipo === 'SA' || objIn.tipo === 'A' || objIn.tipo === 'EA' || objIn.tipo === 'SEA') {
          for (var i = 0; i < obj.length; i++) {
              ...
              if (obj[i].analistaNombre !== null && obj[i].analistaApellidoPaterno !== null && obj[i].analistaApellidoMaterno !== null) {
                  data.push({name: obj[i].analistaNombre + ' ' + obj[i].analistaApellidoPaterno + ' ' + obj[i].analistaApellidoMaterno, y: porc, total: obj[i].total, id: obj[i].idAnalista, tipo: objIn.tipo, idSol: obj[i].estadoSolicitud});
              } else {
    ...
    cargarTerceraGrafica(estatus){
      ...
      let params= {rol: 101,
        empresa: null,
        estadoSolicitud: estatus.idSol,
        idAnalista: estatus.id,
        tipo: 'SAE'};
